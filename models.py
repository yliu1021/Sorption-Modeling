...
def make_vae_encoder(**kwargs):
    ...
    z_mean = Dense(boost_dim, name='z_mean')(x_fc1)
    z_log_var = Dense(boost_dim, name='z_log_var')(x_fc1)

    z = Lambda(sampling, output_shape=(boost_dim,), name='z')([z_mean, z_log_var])

    encoder = Model(inp, [z_mean, z_log_var, z], name='encoder')

    return encoder
    
def make_vae_decoder(**kwargs):
    first_conv_depth = kwargs.get('first_conv_depth', 128)
    pre_deconv1_depth = kwargs.get('pre_deconv1_depth', 128)
    post_deconv2_depth = kwargs.get('post_deconv2_depth', 32)
    last_filter_size = kwargs.get('last_filter_size', 6)
    ...
    latent_inputs = Input(shape=(boost_dim,), name='z_sampling')
    latent_code = Dense(512, use_bias=True, activation='relu')(latent_inputs)
    latent_code = Dense(lc_merge_depth, use_bias=False)(latent_code)
    ...
    model = Model(inputs=[inp, latent_inputs], outputs=[out],
                  name='generator_model')
